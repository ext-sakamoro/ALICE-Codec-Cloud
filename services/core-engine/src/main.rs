    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "codec_engine=info".into()),
        )
        .init();
...
async fn health(State(s): State<Arc<AppState>>) -> Json<Health> {
    ...
    Json(Health {
        ...
        total_ops: st.total_encodes + st.total_decodes + st.total_analyses,
    })
}
...
async fn encode(
    ...
    let bitrate = if dur > 0.0 {
        (enc_size as f64 * 8.0 / dur / 1000.0) as u32
    } else {
        0
    };
...
async fn transcode(
    ...
    let in_size: u64 = 50_000_000;
    let out_size = (in_size as f64 * (100.0 - quality * 0.5) / 100.0) as u64;
    {
        let mut st = s.stats.lock().unwrap();
        st.total_encodes += 1;
        st.bytes_processed += in_size;
    }
    Json(TranscodeResponse {
        job_id: uuid::Uuid::new_v4().to_string(),
        status: "completed".into(),
...
async fn analyze(
    ...
    Json(AnalyzeResponse {
        ...
        bitrate_kbps: 5000,
        duration_secs: dur,
        codec: "3D-Wavelet-rANS".into(),
        wavelet_type: "CDF 9/7".into(),
        entropy_coder: "rANS".into(),
        psnr_db: 42.5,
        ssim: 0.97,
    })
}
...
async fn codecs() -> Json<Vec<CodecInfo>> {
    Json(vec![
        CodecInfo {
            name: "alice-wv".into(),
            description: "3D Wavelet + rANS entropy coding".into(),
            wavelet: "CDF 9/7".into(),
            entropy: "rANS".into(),
            typical_ratio: "20-50x".into(),
            best_for: "High quality video, streaming".into(),
        },
        CodecInfo {
            name: "alice-wv-fast".into(),
            description: "Fast wavelet with Haar basis".into(),
            wavelet: "Haar".into(),
            entropy: "rANS".into(),
            typical_ratio: "10-30x".into(),
            best_for: "Real-time encoding, low latency".into(),
        },
        CodecInfo {
            name: "alice-wv-hq".into(),
            description: "High quality with Daubechies wavelet".into(),
            wavelet: "Daubechies D8".into(),
            entropy: "rANS".into(),
            typical_ratio: "15-40x".into(),
            best_for: "Archival, broadcast quality".into(),
        },
    ])
}
...
async fn stats(State(s): State<Arc<AppState>>) -> Json<StatsResponse> {
    let st = s.stats.lock().unwrap();
    Json(StatsResponse {
        total_encodes: st.total_encodes,
        total_decodes: st.total_decodes,
